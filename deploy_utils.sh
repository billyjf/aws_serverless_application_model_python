...
function zip_push_to_s3_and_generate_deployment_cloudformation() {
    aws cloudformation package --template cloudformation/lambda.yaml --s3-bucket billyjf.carbcounter | grep AWSTemplateFormatVersion --after-context=-1 | tee cloudformation/lambda_deploy.yaml
}
